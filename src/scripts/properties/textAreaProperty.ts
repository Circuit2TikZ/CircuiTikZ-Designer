...
export class TextAreaProperty extends EditableProperty<Text>{
	...
	public buildHTML(): HTMLElement {
		...
		const btnLabelClasses = ["btn","btn-outline-secondary","fs-5","material-symbols-outlined","d-flex","align-items-center","justify-content-center","px-1","flex-grow-1"]
		...
		alignDiv.classList.add("col-12","input-group")
		{
			...
			this.alignLeft.addEventListener("change",ev=>{this.update();Undo.addState()})
			...
			this.alignCenter.addEventListener("change",ev=>{this.update();Undo.addState()})
			...
			this.alignRight.addEventListener("change",ev=>{this.update();Undo.addState()})
		...
		justifyDiv.classList.add("col-12","input-group")
		{
			...
			this.justifyStart.addEventListener("change",ev=>{this.update();Undo.addState()})
			...
			this.justifyCenter.addEventListener("change",ev=>{this.update();Undo.addState()})
			...
			this.justifyEnd.addEventListener("change",ev=>{this.update();Undo.addState()})
